...
const OFFICIAL_FPL_LOGIN_URL = 'https://account.premierleague.com/as/authorize?client_id=bfcbaf69-aade-4c1b-8f00-c1cb8a193030&redirect_uri=https%3A%2F%2Ffantasy.premierleague.com%2F&response_type=code&scope=openid%20profile%20email%20offline_access&language=en';
...
async function loginLegacy(email, password) {
  // Current FPL uses a Premier League-hosted OIDC sign-in. A direct email/password transaction is only retained as an explicit legacy deployment option; the default must not call the retired users.premierleague.com host.
  if (String(process.env.FPL_ENABLE_LEGACY_LOGIN).toLowerCase() !== 'true') {
    return {
      ok: false,
      status: 501,
      code: 'FPL_AUTH_UNSUPPORTED',
      message: 'FPL now uses the official Premier League sign-in page. Open that page to authenticate; this deployment does not have a registered FPL OAuth callback yet.',
      protocol: 'oidc_redirect_required',
      officialLogin: OFFICIAL_FPL_LOGIN_URL,
    };
  }
  ...
  try {
    ...
    if (!cookies) {
      return { ok: false, status: 401, code: 'FPL_AUTH_FAILED', message: 'FPL rejected the supplied credentials.', officialLogin: OFFICIAL_FPL_LOGIN_URL };
    }
    ...
    if (!me.ok || !me.data?.player?.entry) {
      return { ok: false, status: me.status || 401, code: errorCode(me.status, me.error), message: 'FPL login succeeded but the authenticated account could not be read.', officialLogin: OFFICIAL_FPL_LOGIN_URL };
    }
    ...
    return { ok: false, status: 502, code: 'FPL_UNAVAILABLE', message: 'FPL is unavailable while authenticating.', officialLogin: OFFICIAL_FPL_LOGIN_URL };
  }
}
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST' && (route === 'connect' || body.action === 'connect' || body.action === 'login')) {
    ...
    if (!result.ok) return responseError(res, result.status || 401, result.code, result.message, { protocol: result.protocol, officialLogin: result.officialLogin || OFFICIAL_FPL_LOGIN_URL });

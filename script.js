...
async function init() {
  ...
  attachListeners();
  setLP(10, 'FETCHING DATA...');
  let bd = cGet('bootstrap'), fd = cGet('fixtures');
  if (bd && fd) {
    setLP(55, 'LOADING FROM CACHE...');
    S.allFixtures = fd; sortFix(); processBootstrap(bd);
  } else {
    try {
      const [bR, fR] = await Promise.all([fplFetch('/bootstrap-static/'), fplFetch('/fixtures/')]);
      setLP(62, 'PROCESSING...'); bd = await bR.json(); fd = fR.ok ? await fR.json() : [];
      cSet('bootstrap', bd); cSet('fixtures', fd);
      S.allFixtures = fd; sortFix();
      if (!processBootstrap(bd)) return;
    } catch (err) {
      setLP(100, 'ERROR');
      setTimeout(() => showLoadErr(`Could not reach FPL API.<br>${err.message}`), 300);
      return;
    }
  }
  setLP(88, 'BUILDING...'); renderAll(); setLP(100, 'READY');
  const ld = el('liveDot'); if (ld) { ld.classList.add('active'); ld.textContent = 'LIVE'; }
  setTimeout(() => { const ls = el('loadingScreen'); if (!ls) return; ls.style.opacity = '0'; ls.style.transition = 'opacity .35s'; setTimeout(() => ls.remove(), 360); }, 180);
  if (S.fplEntryId) { updateAccountUI(); fetchGWHistory(); }
  fetchLive(); checkPriceChanges();
  setTimeout(() => { renderSeasonPredictor(); renderMarketForecast(); }, 600);
}
...
function buildSquadCtx() {
  ...
  return `GW${S.nextGW || S.currentGW || '?'} squad: ${starters.map(p => `${p.web_name}(${p.posShort},${p.teamShort},form:${p.form},xP:${p.projectedPts},nextFDR:${p.avgFDR.toFixed(1)})`).join(', ')}. Bench: ${bench.map(p => p.web_name).join(', ')}. Captain: ${cap?.web_name || 'none'}. VC: ${vc?.web_name || 'none'}. Value: £${mp.reduce((s,p) => s + p.price, 0).toFixed(1)}m.`;
}
...
async function loadNewsFeed(){
  ...
  area.innerHTML='<div style="text-align:center;padding:1.5rem;color:var(--text-sub)">Loading news...</div>';
  const RSS_FEEDS=[
  ...
  try{
    const results=await Promise.allSettled(RSS_FEEDS.map(async feed=>{
      const r=await fetch(rssProxy(feed.url));if(!r.ok)throw new Error();
      const text=await r.text();const parser=new DOMParser();const doc=parser.parseFromString(text,'text/xml');
      const items=Array.from(doc.querySelectorAll('item')).slice(0,6);
      return items.map(item=>({title:item.querySelector('title')?.textContent||'',link:item.querySelector('link')?.textContent||'#',desc:(item.querySelector('description')?.textContent||'').replace(/<[^>]+>/g,'').slice(0,100),date:item.querySelector('pubDate')?.textContent||'',source:feed.source,isFPL:/(fpl|fantasy|gameweek|transfer|injury)/i.test(item.querySelector('title')?.textContent||'')}));
    }));
    let articles=results.filter(r=>r.status==='fulfilled').flatMap(r=>r.value);articles.sort((a,b)=>new Date(b.date)-new Date(a.date));
    if(!articles.length){area.innerHTML='<div style="color:var(--text-sub);text-align:center;padding:2rem;font-size:.82rem">No news available. Check your connection.</div>';return;}
    area.innerHTML=`<div class="card">${articles.slice(0,15).map(a=>`<div class="news-item" onclick="window.open('${a.link}','_blank')"><div class="news-title">${a.isFPL?'🟢 ':''} ${a.title}</div><div class="news-snippet">${a.desc}...</div><div class="news-meta"><span class="news-source">${a.source}</span><span class="news-tag">${a.date?new Date(a.date).toLocaleDateString('en-GB',{day:'numeric',month:'short'}):''}</span></div></div>`).join('')}</div>`;
  }catch(err){area.innerHTML=`<div style="color:var(--text-sub);text-align:center;padding:2rem;font-size:.82rem">News unavailable: ${err.message}</div>`;}
}

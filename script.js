...
function renderPreviewControls(){const f=el('previewFormationSelect'),inline=el('inlineFormationSelect'),c=el('previewChipSelect'),status=el('chipPreviewStatus');if(f)f.value=legalFormation(S.previewFormation);if(inline)inline.value=legalFormation(S.previewFormation);if(c)c.value=S.previewChip||'none';if(status)status.textContent=S.officialChipUsed?`Official team data · ${({'wildcard':'Wildcard','freehit':'Free Hit','bboost':'Bench Boost','3xc':'Triple Captain'}[S.officialChipUsed]||S.officialChipUsed)} used this gameweek.`:S.previewChip&&S.previewChip!=='none'?`Preview only · ${({'wildcard':'Wildcard','freehit':'Free Hit','bboost':'Bench Boost','3xc':'Triple Captain'}[S.previewChip])} selected.`:'Preview only · official chip usage appears after team sync.';}
/* ══ CAPTAIN AI ═════════════════════════════════════════════════ */

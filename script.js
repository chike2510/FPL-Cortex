...
function pitchCard(p,isBench=false){
  ...
  return`<button type="button" class="pitch-card ${isBench?'is-bench':''} ${selected?'is-swap-selected':''}" data-pid="${p.id}" data-role="${isBench?'bench':'starter'}" aria-label="${p.web_name}, ${isBench?'bench':'starter'}"><span class="pitch-card-top"><span class="pitch-avatar" style="--club:${col.p};--sleeve:${col.s}">${initialsForPlayer(p)}</span>${isC?'<span class="cap-badge">C</span>':''}${isV?'<span class="vc-badge">V</span>':''}</span><span class="pitch-name">${p.web_name}</span><span class="pitch-meta">${p.teamShort} · ${pts!==null?pts+' pts':p.projectedPts+' xP'}</span></button>`;
}
...
function swapSquadPlayers(first,second){
  ...
  ensurePickOrder();const firstOrder=S.pickOrder[first],secondOrder=S.pickOrder[second];
...
function renderTransfers(){
  ...
  const inEl=el('transfersInList'),outEl=el('transfersOutList');if(inEl)inEl.innerHTML=topIn.length?`<div class="market-list">${topIn.map(p=>row(p,'transfers_in_event','var(--green)')).join('')}</div>`:emptyMarket('No market signal','Transfer data is quiet for this gameweek.');if(outEl)outEl.innerHTML=topOut.length?`<div class="market-list">${topOut.map(p=>row(p,'transfers_out_event','var(--red)')).join('')}</div>`:emptyMarket('No market signal','Transfer data is quiet for this gameweek.');
}

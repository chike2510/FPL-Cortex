...
function attachListeners() {
  ...
  el('fplPasswordInput')?.addEventListener('keydown', e => { if(e.key==='Enter') connectFplAccount(); });
  el('connectTeamBtn')?.addEventListener('click', connectFplAccount);
  el('teamIdInput')?.addEventListener('keydown', e => { if(e.key==='Enter') connectFplAccount(); });
  ...
  el('playersRefreshBtn')?.addEventListener('click', refreshData); el('fixturesRefreshBtn')?.addEventListener('click', renderFixtures); el('transfersRefreshBtn')?.addEventListener('click', renderTransferHub); el('intelRefreshBtn')?.addEventListener('click', loadNewsFeed); el('leaguesRefreshBtn')?.addEventListener('click', renderLeagues);
  ...
  el('liveRefreshBtn')?.addEventListener('click', fetchLive); el('simResetBtn')?.addEventListener('click', resetSimulator);
...
async function analyseDraftSquad() {
  ...
  area.innerHTML = `<strong>Draft readout</strong><p>${result.selected.length} players selected with £${result.budget.remaining.toFixed(1)}m remaining. The current XI projects ${projected.toFixed(1)} points including captaincy, with an average fixture difficulty of ${averageFdr.toFixed(1)}. Numerical checks are complete; AI should explain this output rather than replace the constraints.</p>`;
}
...
function renderMyTeam(){
  ...
  setText('squadInfoCount', `${mp.length}/15`); setText('squadInfoValue', `£${mp.reduce((s,p)=>s+p.price,0).toFixed(1)}m`); setText('squadInfoXpts', starters.length ? (starters.reduce((s,p)=>s+p.projectedPts,0)+(cap?cap.projectedPts:0)).toFixed(1) : '—');
  const draftBudget = el('draftBudgetLabel'); if (draftBudget) draftBudget.textContent = `£${mp.reduce((s,p)=>s+p.price,0).toFixed(1)}m / £${squadRules().budget.toFixed(1)}m`;
  renderDraftBuilder();
...
async function loadWeather(){
  ...
  area.innerHTML=`<div class="weather-grid">${cards.map(c=>`<div class="weather-card"><div class="weather-icon">${c.weather.icon}</div><div class="weather-team">${c.short} vs ${c.vs}</div><div style="font-family:var(--font-data);font-size:.58rem;color:var(--text-sub);margin-bottom:3px">${c.stadium}</div><div class="weather-temp">${c.weather.temp}°C</div><div class="weather-desc">${c.weather.desc}</div><div class="weather-wind">${c.weather.wind} km/h wind km/h</div><div class="weather-impact impact-${c.weather.impact.level}">${c.weather.impact.text}</div></div>`).join('')}</div>`;
}
...
function renderDraftArea(){
  ...
  el('applyDraftBtn')?.addEventListener('click',()=>{S.myTeam=S.draftState.myPicks.map(p=>p.id);saveTeam();renderPlayerTable();renderMyTeam();renderDashboard();window.goTab?.('myteam');});
}
...
/* ══ FPL ACCOUNT ════════════════════════════════════════════════ */
function openModal(){const m=el('loginModal');if(m){m.classList.remove('hidden');m.style.display='flex';}if(el('fplEmailInput'))el('fplEmailInput').value='';if(el('fplPasswordInput'))el('fplPasswordInput').value='';clearLoginErr();}
...
async function restoreFplConnection(){
  try{
    ...
    if(!data.connected){S.fplEntryId=null;S.fplPlayer=null;['fpl_entry_id','fpl_player','fpl_leagues'].forEach(k=>localStorage.removeItem(k));updateAccountUI();return;}
...
async function connectFplAccount(){
  const email=el('fplEmailInput')?.value.trim(), password=el('fplPasswordInput')?.value||'', btn=el('connectFplBtn');
  if(!email||!password){setLoginErr('Enter your FPL email and password.');return;}
  clearLoginErr();if(btn){btn.textContent='CONNECTING…';btn.disabled=true;}
  try{
    const {data}=await cortexApi('connect',{method:'POST',body:JSON.stringify({email,password})});
    // The password is discarded immediately and is never written to storage or UI.
    if(el('fplPasswordInput'))el('fplPasswordInput').value='';
    if(!data.ok){
      const messages={FPL_AUTH_FAILED:'FPL rejected those credentials. Check your FPL email and password and try again.',FPL_AUTH_UNSUPPORTED:'The official FPL login flow is not configured on this deployment yet.',FPL_FORBIDDEN:'FPL refused the connection. Check that the account can access its team.',FPL_UNAVAILABLE:'FPL is unavailable right now. Try again in a moment.',FPL_RATE_LIMITED:'FPL is rate-limiting requests. Wait a little and try again.',FPL_TEAM_NOT_FOUND:'FPL authenticated the account, but no team could be loaded.'};
      setLoginErr(messages[data.error]||data.message||'FPL authentication failed.');return;
    }
    const user=data.user||{};S.fplEntryId=user.entry||null;S.fplPlayer={first_name:user.first_name||'',last_name:user.last_name||'',teamName:user.team_name||'',summary_overall_points:user.summary_overall_points,summary_overall_rank:user.summary_overall_rank,summary_event_points:user.summary_event_points,entry:S.fplEntryId};
    if(S.fplEntryId)localStorage.setItem('fpl_entry_id',String(S.fplEntryId));localStorage.setItem('fpl_player',JSON.stringify(S.fplPlayer));
    closeModal();updateAccountUI();
    if(data.team)applyFplTeam(data.team);else await importFplTeam();
    window.goTab?.('myteam');
  }catch(error){setLoginErr('FPL is unavailable right now. Please try again later.');}
  finally{if(btn){btn.textContent='CONNECT FPL ACCOUNT';btn.disabled=false;}}
}

async function searchManager(){
  const id=el('teamIdInput')?.value.trim();
  if(!id||!/^[0-9]+$/.test(id)){setLoginErr('Public entry lookup is not authentication. Connect through the FPL account flow above.');return;}
  const res=await fplFetch(`/entry/${id}/`);
  if(!res.ok){setLoginErr('No public FPL entry was found for that ID.');return;}
  const data=await res.json();
  setLoginErr(`Public entry found for ${data.player_first_name||'manager'}; it is not connected. Use FPL account authentication to load the official team.`);
}
...
function updateAccountUI(){
  ...
  if(S.fplPlayer){
    ...
    if(ds) ds.textContent='FPL Entry: '+S.fplEntryId;
    ...
    if(ab){ab.style.display='flex';
      ...
      const mt=el('fplTeamMeta');if(mt)mt.textContent='FPL Entry: '+S.fplEntryId;
    }
    // Update AI hero

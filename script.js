...
// Public FPL data — ALWAYS goes browser → corsproxy → FPL
// This bypasses Vercel server IP blocks entirely.
function fplURL(fplPath) {
  return `${CORS}${encodeURIComponent(`${FPL}${fplPath}`)}`;
}
...
async function fetchBootstrap() {
  try {
    const res = await fetch(fplURL('/bootstrap-static/'));
...
async function fetchFixtures() {
  try {
    const res = await fetch(fplURL('/fixtures/'));
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
...
async function fetchLive() {
  ...
  try {
    ...
    const url = fplURL(`/event/${gw}/live/`);
...
async function connectEntry(entryId) {
  ...
  const res = await fetch(fplURL(`/entry/${entryId}/`));
  if (!res.ok) {
...
async function searchManager() {
  ...
  try {
    ...
    const res = await fetch(fplURL(`/search/?q=${encodeURIComponent(query)}&page_size=8`));
...
async function importFplTeam() {
  ...
  try {
    const res = await fetch(fplURL(`/entry/${S.fplEntryId}/event/${gw}/picks/`));
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data  = await res.json();
...
async function loadLeaguesList() {
  ...
  try {
    const res = await fetch(fplURL(`/entry/${S.fplEntryId}/`));
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
...
async function loadStandings(leagueId, type, name, page=1) {
  ...
  try {
    ...
    const res = await fetch(fplURL(endpoint));
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();

...
function startCompanionConnection(){const btn=el('companionConnectBtn'),status=el('companionStatus');if(btn){btn.disabled=true;btn.textContent='OPENING FPL…';}if(status)status.textContent='You will return here after signing in.';window.postMessage({type:'CORTEX_START_OFFICIAL_AUTH'},location.origin);window.setTimeout(()=>{if(status&&status.textContent==='You will return here after signing in.')status.textContent='Still waiting. If no account appears, install or enable the Cortex browser companion and try again.';if(btn){btn.disabled=false;btn.textContent='TRY OFFICIAL FPL AGAIN';}},90000);}

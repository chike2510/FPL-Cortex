...
/* ══ KEYS ══════════════════════════════════════════════════════ */
const GROQ_KEY   = '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';
const WEATHER_KEY= '9c5f96ac1ffc93da68ec0f2b992e62fc';
...
function switchTab(name) {
  ...
  const map = {
    ...
    scout:()=>{ renderPriceChanges(); renderDifferentials(); },
...
async function loadWeather(){
  ...
  const results=await Promise.allSettled(toFetch.slice(0,6).map(async item=>{const st=STADIUMS[item.short];if(!st)return null;const r=await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${st.lat}&lon=${st.lon}&appid=${WEATHER_KEY}&units=metric`);if(!r.ok)throw new Error();const d=await r.json();return{...item,weather:{temp:Math.round(d.main.temp),desc:d.weather[0].description,wind:Math.round(d.wind.speed*3.6),icon:getWeatherIcon(d.weather[0].main),impact:getWeatherImpact(d.wind.speed,d.weather[0].main)},stadium:st.name};}));
...
function renderXGStats() {
  ...
  const players = S.players.filter(p => p.minutes > 180 && (p.element_type === 3 || p.element_type === 4))
    .sort((a,b) => parseFloat(b.expected_goals||0) - parseFloat(a.expected_goals||0)).slice(0,15);
  if (!players.length) { area.innerHTML = '<div style="color:var(--text-sub);padding:1rem;font-size:.82rem">No xG data available.</div>'; return; }
  area.innerHTML = `<div class="player-table-wrap"><table class="player-table">
    <thead><tr><th>Player</th><th>xG</th><th>Goals</th><th>Diff</th><th>xA</th><th>Assists</th><th>Diff</th></tr></thead>
    <tbody>${players.map(p => {
      const xg = parseFloat(p.expected_goals||0).toFixed(1), goals = p.goals_scored;
      const xa = parseFloat(p.expected_assists||0).toFixed(1), assists = p.assists;
      const gDiff = goals - parseFloat(xg), aDiff = assists - parseFloat(xa);
      const gColor = gDiff > 1 ? 'var(--amber)' : gDiff < -1 ? 'var(--blue)' : 'var(--text)';
      const aColor = aDiff > 1 ? 'var(--amber)' : aDiff < -1 ? 'var(--blue)' : 'var(--text)';
      return `<tr>
        <td><div class="player-name">${p.web_name}</div><div class="player-sub">${p.teamShort}</div></td>
        <td style="font-family:var(--font-data);color:var(--blue)">${xg}</td>
        <td style="font-family:var(--font-data);font-weight:700">${goals}</td>
        <td style="font-family:var(--font-data);color:${gColor}">${gDiff>0?'+':''}${gDiff.toFixed(1)}</td>
        <td style="font-family:var(--font-data);color:var(--blue)">${xa}</td>
        <td style="font-family:var(--font-data);font-weight:700">${assists}</td>
        <td style="font-family:var(--font-data);color:${aColor}">${aDiff>0?'+':''}${aDiff.toFixed(1)}</td>
      </tr>`;
    }).join('')}</tbody>
  </table></div>
  <div style="margin-top:.5rem;font-family:var(--font-data);font-size:.58rem;color:var(--text-sub)">
    <span style="color:var(--amber)">+N = Overperforming xG (lucky/clinical)</span> · <span style="color:var(--blue)">-N = Underperforming (due a goal)</span>
  </div>`;
}
...
function renderPricePrediction() {
  ...
  const rising = S.players
    .filter(p => p.transfers_in_event > p.transfers_out_event && p.cost_change_event === 0)
  ...
  const falling = S.players
    .filter(p => (p.transfers_out_event||0) > p.transfers_in_event && p.cost_change_event === 0)
...
function renderTeamForm() {
  ...
  const teamForm = Object.values(S.teams).map(team => {
...
/* ══ HOOK NEW FEATURES INTO EXISTING FLOW ═══════════════════════ */
// Patch attachListeners to add new feature listeners
const _origAttachListeners = attachListeners;
function attachListeners() {
  _origAttachListeners();
  // Voice input
  el('voiceBtn')?.addEventListener('click', toggleVoice);
  initVoiceInput();
  // Cortex score tap to explain
  el('cortexScoreCard')?.addEventListener('click', () => {
    const area = el('cortexExplainArea');
    if (area) { area.style.display = area.style.display === 'none' ? 'block' : 'none'; if (area.style.display === 'block') explainCortexScore(); }
  });
  // GW Preview
  ...
  // Transfer planner
  el('transferPlannerTab')?.addEventListener('click', renderTransferPlanner);
  // Shortlist
...
// Patch switchTab to include new tabs
...
// Patch init to run extra features
const _origInit = init;
async function init() {
  await _origInit();
  initDashboardWidgets();
  setTimeout(renderShortlist, 800);
}

// Add timeline + card generator to global click handler
const _origGlobalClick = handleGlobalClick;
function handleGlobalClick(e) {
  _origGlobalClick(e);
  const timelineBtn = e.target.closest('.timeline-btn');
  if (timelineBtn) { showPlayerTimeline(parseInt(timelineBtn.dataset.pid)); return; }
  const cardBtn = e.target.closest('.card-gen-btn');
  if (cardBtn) { generatePlayerCard(parseInt(cardBtn.dataset.pid)); return; }

...
function setFplCookie(c) { _fplCookie = c; localStorage.setItem('fpl_cookie', c); }
function loadFplCookie() { _fplCookie = localStorage.getItem('fpl_cookie') || ''; }
loadFplCookie();
...
function attachListeners() {
  ...
  el('connectTeamBtn')?.addEventListener('click', submitTeamId);
  el('teamIdInput')?.addEventListener('keydown', e => { if(e.key==='Enter') submitTeamId(); });
...
function handleAccountBtn(){if(S.fplEntryId) window.goTab?.('leagues'); else openModal();}

async function submitTeamId() {
  const id = parseInt(el('teamIdInput')?.value || '');
  const err = el('loginError');
  if (!id || isNaN(id)) {
    if (err) { err.textContent = 'Please enter a valid Team ID'; err.style.display = 'block'; }
    return;
  }
  S.fplEntryId = id;
  localStorage.setItem('fpl_entry_id', String(id));
  // Close modal
  const m = el('loginModal');
  if (m) { m.classList.add('hidden'); m.style.display = 'none'; }
  // Load team data
  if (typeof fetchEntry === 'function') fetchEntry();
  else if (typeof loadFplEntry === 'function') loadFplEntry();
}
...
function updateAccountUI(){
  ...
  if(lbl) lbl.textContent = S.fplPlayer ? (S.fplPlayer.player_first_name||'') : '';
  ...
  if(S.fplPlayer){
    if(dn) dn.textContent=S.fplPlayer.player_first_name+' '+S.fplPlayer.player_last_name;
    if(ds) ds.textContent='Team ID: '+S.fplEntryId;
    ...
    if(ab){ab.style.display='flex';
      const mn=el('fplManagerName');if(mn)mn.textContent=S.fplPlayer.player_first_name+' '+S.fplPlayer.player_last_name;
      const mt=el('fplTeamMeta');if(mt)mt.textContent='Team ID: '+S.fplEntryId;
    }
    // Update AI hero
...
async function importFplTeam(){
  ...
  // Populate myPicks for Score Simulator — enrich with isBench flag
  S.myPicks=picks.map(pk=>({...pk,isBench:pk.position>11}));
  saveTeam();renderAll();}

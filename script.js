...
/* ══ KEYS ══════════════════════════════════════════════════════ */
// ⚠️  API keys removed from client — all AI/weather calls go through /api/* serverless functions
...
async function fplFetch(path) {
  // Step 1: try our own /api/proxy first — fastest and most reliable
  try {
    return await fetchWithTimeout(`/api/proxy?path=${encodeURIComponent(path)}`, 6000);
  } catch (_) {
    // fall through to public proxies
  }
  // Step 2: race public proxies simultaneously
  const publicProxies = [
    `https://corsproxy.io/?${encodeURIComponent(`${FPL}${path}`)}`,
    `https://api.allorigins.win/raw?url=${encodeURIComponent(`${FPL}${path}`)}`,
    `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(`${FPL}${path}`)}`,
  ];
  try {
    return await Promise.any(publicProxies.map(url => fetchWithTimeout(url, 7000)));
  } catch {
...
/* ══ CLAUDE AI (via /api/ai serverless — key never exposed to client) ══ */
async function groqChat(messages, maxTokens = 500) {
  // Maintain same call signature so all existing callers work unchanged
  // Strip system message from array — pass it separately as 'system' field
  ...
  if (!res.ok) {
    ...
    throw new Error(err.error || `AI ${res.status}`);
  }
...
async function loadNewsFeed() {
  ...
  // Each feed is individually guarded — one failure can't kill the whole tab
  const fetchFeed = async (feed) => {
    try {
      const r = await fetch(rssProxy(feed.url), { signal: AbortSignal.timeout(8000) });
      if (!r.ok) return { feed, ok: false, articles: [] };
      const text = await r.text();
      const doc = new DOMParser().parseFromString(text, 'text/xml');
      if (doc.querySelector('parsererror')) return { feed, ok: false, articles: [] };
      const articles = Array.from(doc.querySelectorAll('item')).slice(0, 5).map(item => ({
      ...
      return { feed, ok: true, articles };
    } catch {
      return { feed, ok: false, articles: [] };
    }
  };
  ...
  const sourceStats = results.map(r => ({ feed: r.feed, ok: r.ok, count: r.articles.length }));

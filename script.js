...
/* ══ FPL ACCOUNT ════════════════════════════════════════════════ */
function openModal(){const m=el('loginModal');if(m){m.classList.remove('hidden');m.style.display='flex';}if(el('fplEmailInput'))el('fplEmailInput').value='';if(el('fplPasswordInput'))el('fplPasswordInput').value='';toggleLoginMode('fpl');clearLoginErr();}
...
function toggleLoginMode(mode){const f=el('fplLoginPanel'),t=el('teamLoginPanel'),fb=el('loginModeFpl'),tb=el('loginModeTeam');const isFpl=mode!=='team';if(f)f.style.display=isFpl?'':'none';if(t)t.style.display=isFpl?'none':'';fb?.classList.toggle('active',isFpl);tb?.classList.toggle('active',!isFpl);}
...
async function connectFplAccount(){
  ...
  try{
    ...
    if(!data.ok){
      const messages={FPL_AUTH_FAILED:'FPL rejected the supplied credentials.',FPL_AUTH_UNSUPPORTED:'Use the official FPL login page. This deployment does not have a registered OAuth callback yet.',FPL_FORBIDDEN:'FPL refused the connection. Check that the account can access its team.',FPL_UNAVAILABLE:'FPL is unavailable right now. Try again in a moment.',FPL_RATE_LIMITED:'FPL is rate-limiting requests. Wait a little and try again.',FPL_TEAM_NOT_FOUND:'FPL authenticated the account, but no team could be loaded.'};

...
function hideLogo() {
  ...
  const ld=el('liveDot'); if(ld) ld.style.display=''; const lb=el('liveBadgeText'); if(lb) lb.style.display='';
}
...
function updateAccountUI(){
  ...
  if(lbl) lbl.textContent = S.fplPlayer ? (S.fplPlayer.first_name||'ACCOUNT') : '';
  const btn=el('menuBtn');
  if(btn) btn.classList.toggle('logged-in',!!S.fplPlayer);
  // Update auth modals
  if(window.updateUserUI) return;
  const dn=el('ddName'),ds=el('ddSub'),da=el('ddAvatar'),dsb=el('ddSignBtn'),dso=el('ddSignOut');
  if(S.fplPlayer){
    if(dn) dn.textContent=S.fplPlayer.player_first_name||S.fplPlayer.first_name||'Manager';

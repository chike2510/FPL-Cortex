...
async function connectFplAccount(){
  ...
  try{
    ...
    if(!data.ok){setLoginErr(data.message||'FPL authentication failed.');return;}
    ...
    window.goTab?.('myteam');
  }catch(error){setLoginErr('FPL is unavailable. Please try again later.');}
...
function updateAccountUI(){
  ...
  if(S.fplPlayer){
    ...
    if(ds) ds.textContent='Sign in to unlock Pro features';

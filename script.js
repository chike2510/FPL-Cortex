...
function renderDraftArea(){
  ...
  if(!d.active){area.innerHTML='<div class="empty-state"><span class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 6h16v12H4z"/><path d="M8 6V4h8v2M8 10h8M8 14h5"/></svg></span><h3>Build a watchlist, then start</h3><p>Use the player pool to set priorities. Start a mock room when you are ready.</p></div>';setText('draftStatus','Mock mode · no live league sync');return;}
  ...
  setText('draftStatus',complete?'Mock complete':myTurn?'Your pick':'AI picking…');setText('draftRoomTitle',complete?'Draft squad ready':`Round ${d.round} · ${myTurn?'Your pick':'AI pick'}`);
}
...
function resetCompanionUI(){const status=el('companionStatus'),card=el('companionUserCard'),confirm=el('companionConfirmBtn'),btn=el('companionConnectBtn');if(status)status.textContent='Ready when you are.';if(card){card.hidden=true;card.innerHTML='';}if(confirm){confirm.hidden=true;confirm.disabled=false;}if(btn){btn.disabled=false;btn.textContent='CONTINUE TO OFFICIAL FPL';}S.companionUser=null;}
function startCompanionConnection(){const btn=el('companionConnectBtn'),status=el('companionStatus');if(btn){btn.disabled=true;btn.textContent='OPENING FPL…';}if(status)status.textContent='You will return here after signing in.';window.postMessage({type:'CORTEX_REQUEST_FPL_SESSION'},location.origin);try{window.open('https://fantasy.premierleague.com/','_blank','noopener,noreferrer');}catch(_){}window.setTimeout(()=>{if(status&&status.textContent==='You will return here after signing in.')status.textContent='Still waiting. If no account appears, install or enable the Cortex browser companion and try again.';if(btn){btn.disabled=false;btn.textContent='TRY OFFICIAL FPL AGAIN';}},90000);}

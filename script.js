 ...
 *
 * Modes:
 *  - On Vercel (HTTPS): uses /api/ serverless functions
 *  - Local / Acode (file:// or HTTP): uses corsproxy.io → FPL directly
 */
...
function fplURL(vercelPath, fplPath) {
  return IS_VERCEL ? vercelPath : `${CORS}${encodeURIComponent(`${FPL}${fplPath}`)}`;
}
...
async function fetchBootstrap() {
  try {
    const res = await fetch(fplURL('/api/players', '/bootstrap-static/'));
...
async function fetchFixtures() {
  try {
    const res = await fetch(fplURL('/api/fixtures', '/fixtures/'));
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
...
async function fetchLive() {
  ...
  try {
    const url = IS_VERCEL
      ? `/api/live?gw=${gw}`
      : `${CORS}${encodeURIComponent(`${FPL}/event/${gw}/live/`)}`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const raw = await res.json();

    if (IS_VERCEL) {
      S.liveData = raw.elements;
    } else {
      const map = {};
...
function handleAccountBtn() {
  if (S.fplCookie) {
    // already logged in — show option to go to leagues
    switchTab('leagues');
  } else {
    openLoginModal();
  }
...
async function submitLogin() {
  ...
  if (!IS_VERCEL)    { setLoginError('Login only works when deployed on Vercel. Use the manual squad builder while testing locally.'); return; }
  ...
  try {
    ...
    // Save session
    S.fplCookie  = data.cookie;
    S.fplEntryId = data.entryId;
    S.fplPlayer  = data.player;
    localStorage.setItem('fpl_cookie',   S.fplCookie);
    ...
    // Auto-import team if entry found
    if (S.fplEntryId) importFplTeam();

  } catch (err) {
    setLoginError(`Request failed: ${err.message}`);
  } finally {
    if (btn) { btn.textContent='LOGIN TO FPL'; btn.disabled=false; }
  }
}
...
async function importFplTeam() {
  if (!S.fplEntryId) { alert('Please log in with your FPL account first.'); return; }
  if (!IS_VERCEL)    { alert('Team import only works on Vercel.'); return; }

  const gw = S.currentGW || S.nextGW;
  try {
    const res = await fetch(`/api/myteam?entry=${S.fplEntryId}&gw=${gw}`, {
      headers: { 'x-fpl-cookie': S.fplCookie||'' },
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    const picks = data.picks || [];

    if (!picks.length) { alert('No picks found for this gameweek yet.'); return; }

    // Map FPL picks to player IDs
    const newTeam = picks.map(pk=>pk.element).filter(id=>S.players.find(p=>p.id===id));
    if (!newTeam.length) { alert('Could not match picks to loaded player data.'); return; }
    ...
    saveTeam();
    renderAll();
    alert(`✅ Imported ${newTeam.length} players from your FPL account!`);

  } catch (err) {
    alert(`Failed to import team: ${err.message}`);
  }
}
...
async function loadLeaguesList() {
  if (!S.fplEntryId || !IS_VERCEL) {
    setHTML('classicLeaguesList', `<div style="color:var(--text-sub);font-size:0.8rem;padding:0.5rem;">League data requires Vercel deployment.</div>`);
    return;
  }

  setHTML('classicLeaguesList', '<div style="color:var(--text-sub);font-size:0.8rem;padding:0.5rem;">Loading...</div>');
  setHTML('h2hLeaguesList',     '<div style="color:var(--text-sub);font-size:0.8rem;padding:0.5rem;">Loading...</div>');

  try {
    const res = await fetch(`/api/leagues?entry=${S.fplEntryId}`, {
      headers: { 'x-fpl-cookie': S.fplCookie||'' },
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    ...
    const renderList = (list, elId) => {
      ...
      el.innerHTML = list.map(l=>`
        <div class="league-item" data-lid="${l.id}" data-name="${l.name||l.league_name}">
          <div>
            <div class="league-name">${l.name||l.league_name}</div>
            <div class="league-meta">ID: ${l.id} · Entry rank: ${l.entry_rank?.toLocaleString()||'—'}</div>
          </div>
          <span class="league-arrow">›</span>
        </div>`).join('');
    };

    renderList(data.classic || [], 'classicLeaguesList');
    renderList(data.h2h     || [], 'h2hLeaguesList');

  } catch (err) {
    setHTML('classicLeaguesList', `<div style="color:var(--red);font-size:0.8rem;padding:0.5rem;">Failed to load: ${err.message}</div>`);
  }
}

async function loadStandings(leagueId, type, name, page=1) {
  if (!IS_VERCEL) { alert('Standings require Vercel deployment.'); return; }
  ...
  try {
    const res = await fetch(`/api/leagues?type=${type}&id=${leagueId}&page=${page}`, {
      headers: { 'x-fpl-cookie': S.fplCookie||'' },
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    ...
    if (table) table.innerHTML = `
      ...
      ${rows.map(r => {
        ...
        const rankClass = r.rank<=3?'top3':'';
        const move = r.last_rank - r.rank;
        ...
        return `
          <div class="standings-row ${isMine?'my-entry':''}">
            <div class="standings-rank ${rankClass}">${r.rank}</div>
            <div>
    ...
    if (pag) {
      ...
      if (data.has_next)  ph+=`<button class="page-btn" data-page="${page+1}">Next ›</button>`;
      pag.innerHTML = ph;
    }

  } catch (err) {
    if (table) table.innerHTML = `<div style="padding:1rem;color:var(--red);">Failed to load: ${err.message}</div>`;
  }
}

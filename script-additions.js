...
(function () {
  ...
  function renderGWPlanner() {
    ...
    const avgFDR = gw => {
      const f = bs.fixtures?.filter(fix => fix.event === gw.id);
    ...
    const captains = gws.map(gw => {
      const best = topByForm.find(p => {
        ...
        const hasFixture = bs.fixtures?.some(f =>
  ...
  function initSellOrHold() {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!q) return;
      const S = window.S || {};
      const players = S.bootstrap?.elements || [];
      const p = players.find(pl => pl.web_name.toLowerCase().includes(q) || (pl.first_name + ' ' + pl.second_name).toLowerCase().includes(q));
  ...
  function renderSimulator() {
    ...
    const starters = mp.filter(p => !p.isBench).slice(0, 11);
    if (!starters.length) {
